...
sha000016 <- read.csv("F:/HW1/000016.csv")
sha000016$date = as.POSIXct(sha000016$date)
sha000016 <- subset.data.frame(sha000016, 
                               (date >= as.POSIXct("2015-01-01") & 
                                (date <= as.POSIXct("2015-09-30"))),
                               select = c(date,Code,Close,PreClose))
sha000016$Rate = (sha000016$Close-sha000016$PreClose) / sha000016$PreClose
